...
#   4. /run/test-pivot-root /run/newroot  → does pivot_root + lazy umount
...
PIVOT_BIN="../test-pivot-root"
[ -x "$PIVOT_BIN" ] || { echo "build $PIVOT_BIN first (cc -o $PIVOT_BIN ../test-pivot-root.c)" >&2; exit 1; }

SSH_OPTS=( -i keys/assix_id_ed25519 -p 2222 -q
           -o StrictHostKeyChecking=no -o UserKnownHostsFile=/dev/null
           -o LogLevel=ERROR )

echo "[swap-pkg-pivot] tar-piping bits + pivot binary into VM /run"
tar c \
    --transform="s|.*/test-pivot-root$|test-pivot-root|" \
    --transform="s|^images/||" \
    "$IMG" "$HASH" "$RH" "$PIVOT_BIN" \
  | ssh "${SSH_OPTS[@]}" root@127.0.0.1 \
        "tar x -C /run/ && chmod +x /run/test-pivot-root"
...
mkdir -p /run/assix-pivot/upper /run/assix-pivot/work

echo "[in-vm] staging new root at ${NEWROOT}"
mkdir -p "$NEWROOT"
mount -t overlay -o "lowerdir=${LOWERSPEC},upperdir=/run/assix-pivot/upper,workdir=/run/assix-pivot/work" \
    overlay "$NEWROOT"
...
echo "[in-vm] /run/test-pivot-root ${NEWROOT}"
chmod +x /run/test-pivot-root
/run/test-pivot-root "$NEWROOT"
...
echo
echo "[swap-pkg-pivot] done. Probe with: ssh -i keys/assix_id_ed25519 -p 2222 root@127.0.0.1"
