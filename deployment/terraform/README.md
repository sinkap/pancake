...
Engine, with Cloud SQL Postgres for persistence. Two sibling directories
under `deployment/`:

- `deployment/terraform/` — provisions the GKE Autopilot cluster, Cloud
  SQL instance, service accounts, IAM bindings, CAS pool, and a private
  build-server VM.
- `deployment/k8s/` — Kubernetes manifests (Deployment, Service,
  Ingress, secret templates) for running the fleet server on top of the
  cluster.

The manifests are templates: every `PROJECT_ID`, `REGION`,
`INSTANCE_NAME`, or `EXAMPLE.COM` placeholder must be replaced before
`kubectl apply`.
...
# 3. Build + push the fleet-server image (from the repo root)
cd ../..
...
│   ├── sign-server/Dockerfile
│   ├── ca-server/{Dockerfile,init.sh}
│   ├── compose.yaml
│   └── fleet-compose.yaml
├── k8s/
│   ├── namespace.yaml
...
│   ├── fleet-server-service.yaml      # ClusterIP for HTTP + gRPC + internal LB
│   └── ingress.yaml                   # GKE Ingress + managed cert
└── terraform/
    ├── main.tf                        # GKE cluster
    ├── cloudsql.tf                    # Cloud SQL Postgres
    ├── networking.tf                  # VPC + firewall
    ├── cas.tf                         # Google CAS pool (cert issuance)
    ├── iam.tf                         # service accounts + bindings
    ├── build_server_vm.tf             # private build-server VM
    ├── variables.tf
    └── outputs.tf
```
